...
it('casts entry fields to enums and dates', function (): void {
    ...
    expect($entry->type)->toBe(EntryType::Drug)
        ->and($entry->status)->toBe(EntryStatus::Documented)
        ->and($entry->approved_at)->not->toBeNull()
        ->and($entry->archived_at)->not->toBeNull();
});

it('defines the canonical entry status values', function (): void {
    expect(array_map(
        fn (EntryStatus $status): string => $status->value,
        EntryStatus::cases(),
    ))->toBe([
        'draft',
        'documented',
...
it('filters active archived approved status and type scopes', function (): void {
    Entry::factory()->type(EntryType::Drug)->status(EntryStatus::Documented)->create();
    ...
    expect(Entry::query()->active()->count())->toBe(2)
        ...
        ->and(Entry::query()->status(EntryStatus::Documented)->count())->toBe(1)
